...
public class Mainservice {
	...
	public List<CustomData> WeatherService(String city) {
		...
		for (int i = 1; i <= 3; i++) {
			...
			String apiUrl = "https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst?ServiceKey=yOUYauWN3w884SILQEEd4%2FJ5tIP6XGff2M4%2BocNwA086Xmg0y%2FFqjlw4x0r0SUoQXYDdzcKIavB5rXfMHaVMlQ%3D%3D&pageNo=1&numOfRows=12&dataType=JSON&base_date="
					+ currentDateTime1.format(formatter) + "&base_time=" + formattedTime + "&nx=" + cityX + "&ny="
					+ cityY + "&ftype=SHRT";
			...
			try {
				...
				if (responseCode == HttpURLConnection.HTTP_OK) {
					...
					// 응답 확인
					System.out
							.println("Response Header: " + responseWrapperDTO.getResponse().getHeader().getResultMsg());
